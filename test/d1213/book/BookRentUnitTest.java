...
class BookRentUnitTest {
	//아래의 dao클래스 메소드를 테스트 하는 것이 목적.
	private BookRentDao dao = BookRentDao.getInstance();

	@Order(1)
	@DisplayName("대여중인 도서 목록의 크기는 count 함수값과 동일해야한다.")
	@Test
	void selectRentBookTest() throws SQLException {
		...
		//assertxxx 은 검증 메소드 : 기대가 일치하면 성공, 그렇지 않으면 실패
		//junit 실행결과 창에 Failures(실패) 숫자가 1이면 테스트 메소드 오류
//		fail("Not yet implemented");	//fail 메소드는 검증 실패
	}
	
	@Order(2)
	@DisplayName("회원번호 10004 도서 대여 가능여부 - 기대값 false")
	@Test
...
//		assertFalse(result);	//위와 동일한 메소드
	...
	@Order(3)
	@DisplayName("도서코드 여러개 데이터에 대한 대여 가능여부 - 기대값 false")
	@ParameterizedTest
	@ValueSource(strings = {"B1101", "A1101", "A1102"})
//	@Test
	void isAvailableBookTest(String bcode) throws SQLException {	//여러개 도서코드 받을 매개변수
		boolean result = dao.isAvailableBook(bcode);
		assertFalse(result);
	//주의할점 : 정확한 데이터와 정확한 기대값으로 테스트를 했을 때
	//			실패한다면 실행코드(메소드)가 오류 입니다.
	}
	
	@Order(4)
	@DisplayName("회원번호 여러개 데이터에 대한 대여 가능여부 - 기대값 true")
	@ParameterizedTest
	...
	@Order(5)
	@DisplayName("회원번호 10002의 대여 정보가 있으므로 - 기대값 not null")
	@Test
...
/*
	...
	단위테스트는 데이터가준비(given)되면 어떤 메소드를 실행(when)시켰을 때 기대하는(then) 결과 값이 나와야한다.
	검증하는 과정이다.
