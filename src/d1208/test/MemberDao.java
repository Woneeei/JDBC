...
//		ㄴ 1개의 객체로 여러 프로그램에서 사용하도록 합니다. static 안쓰고 싱글턴으로 만듦
//		ㄴ인스턴스 메소드로 만들어야 인터페이스 적용과 상속에서 메소드 재정의가 가능합니다.
public class MemberDao {
	
	public static MemberDao dao = new MemberDao();	// private static 멤버변수가 참조하는 것은 자기자신 객체
	private MemberDao() { };						// 생성자 private (이 클래스 외에는 사용못함)
	public static MemberDao getMemberDao() {		//생성된 객체를 리턴해 주는 public메소드 - 항상 동일한 객체를 리턴
		return dao;
	}
...
//		conn.rollback();    //하나의 트랜잭션을 구성하는 여러 SQL 중 일부 오류일때. 이미실행된 SQL은 취소합니다.
	...
	public List<Member> selectList() throws SQLException {	//Member객체를 저장한 리스트를 리턴
		List<Member> list = null;
		
		Connection conn = OracleUtil.getConnection(); //conn 지역변수로 사용
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String sql = "select * from TBL_MAMBERSHIP";
		
		pstmt = conn.prepareStatement(sql);	//Exception 처리는 jvm에게 throw (위임.넘기기)
		rs = pstmt.executeQuery();
		List<Member> memberList = new ArrayList<>();	
		//출력이 목적이 아니라 select 조회결과를 자바 객체로 저장라는게 목적
		while(rs.next()) {
			Member m = new Member(
					rs.getInt(1),
					rs.getString(2),
	...
	public Member selectOne(int custno) throws SQLException {	
		//PK 값 custno 를 인자로 전달 받아서 해당되는 조회결과를 Member객체로 리턴
		Member member = null;
		Connection conn = OracleUtil.getConnection(); //conn 지역변수로 사용
		PreparedStatement pstmt = null;
		ResultSet rs = null;
