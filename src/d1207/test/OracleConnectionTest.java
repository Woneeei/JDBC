...
public class OracleConnectionTest {
	...
	public static void main(String[] args) {
		...
		Connection conn = null;		//Connection 인터페이스(같은 종류임을 표시하는)

		//db접속 정보
		String url = "jdbc:oracle:thin:@192.168.0.152:1521:XE";   //데이터베이스 URL - 디비버에서 확인가능합니다. - jdbc:oracle:thin는 지정된 프로토콜
	...
	//									ㄴ new 연산 외에 객체를 생성하는 다른 방법 : 메소드를 실행하여	객체생성하는 디자인패턴
		...
		} finally {	//최종적으로 자원(리소스)을 해제할 때 finally안에 작성함
			//db접속 후 모든 db작업을 종료하면 접속도 종료
			try {
				conn.close();
			} catch (SQLException e) { }
		}
